...
class CheckInVCB: UIViewController {
    
    
    //MARK: - Items for container
    let imageContainer: UIImageView = {
        let image = UIImageView(image: #imageLiteral(resourceName: "shower"))
    ...
    let textContainer: UITextView = {
        ...
        let attributedText = NSMutableAttributedString(string: "Mandi", attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 36, weight: .heavy)])
        
        attributedText.append(NSAttributedString(string: "\n\nVirus dapat dengan segera dihilangkan dengan sabun, kami sarankan Anda untuk segera mandi agar tetap bersih dari virus.", attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18, weight: .regular)]))
